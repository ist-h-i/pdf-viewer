...
### NFR-02/NFR-03: エラー耐性・性能（未確認）
- 破損/非 PDF の読み込み時にエラー表示し、アプリが落ちない（状態クリア/再選択ができる）
- 中規模 PDF のレンダリング/検索が段階的に更新され、UI が固まらない（必要なら処理分割/進捗表示を検討）
...
### Phase 5: 非機能要件の補強（NFR）
1. 破損/非 PDF の読み込み時のエラー表示と復帰導線（状態クリア/再選択）
2. 中規模 PDF のレンダリング/検索の段階更新を確認し、必要なら処理分割/進捗表示を追加

**受け入れ基準**
- 破損/非 PDF を読み込んでもアプリが落ちず、エラー表示される（再選択できる）
- 中規模 PDF の表示/検索がフリーズせず、段階的に更新される
...
- 破損/非 PDF の読み込みでエラー表示され、アプリが落ちない
- 中規模 PDF の表示/検索が固まらず段階的に更新される
